...
namespace test
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (cbPython.Checked)
                selectedSubjects.Add(cbPython.Text);
            if (cbC.Checked)
                selectedSubjects.Add(cbC.Text);
            if (cbCpp.Checked)
                selectedSubjects.Add(cbCpp.Text);
            if (cbCs.Checked)
                selectedSubjects.Add(cbCs.Text);
            if (cbJ.Checked)
                selectedSubjects.Add(cbJ.Text);
            result += string.Join(", ", selectedSubjects);
